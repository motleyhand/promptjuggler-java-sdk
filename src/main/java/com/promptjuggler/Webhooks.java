...
public final class Webhooks {
   ...
   * @param signatureHeader the {@code PromptJuggler-Signature} header value
   *        ({@code t=<ts>,v1=<hmac>}); a null/absent header verifies as false
  ...
  public static boolean verifySignature(String payload, @Nullable String signatureHeader,
      String secret) {
    return verifySignature(payload, signatureHeader, secret, 300,
   ...
   *
   * <p>
   * Recomputes the HMAC-SHA256 of {@code <timestamp>.<payload>}, compares it in constant time, and
   * rejects deliveries whose timestamp falls outside {@code tolerance} seconds of {@code now}.
   */
  public static boolean verifySignature(String payload, @Nullable String signatureHeader,
      String secret, long tolerance, long now) {
